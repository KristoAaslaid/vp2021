<?php
    ...
    function sign_in($email, $password){
        ...
        if($stmt->fetch()){
            ...
            if(password_verify($password, $password_from_db)){
				...
				//siin edaspidi sisselogimisel pärime SQL-iga kasutajaprofiili, kui see on olemas,
				//siis loeme sealt tausta- ja tekstivärvid, muidu kasutame mingeid vaikevärve
	...
	function store_profile($description, $bg_color, $text_color){
		...
		echo $conn->error;
		$stmt->bind_param("sss", $description, $bg_color, $text_color);
		$stmt->bind_result($id); // Vaata seda kohta just tema git'ist, sest see funktsioon on suht vähk ngl
		$success = null;
        if($stmt->execute()){
